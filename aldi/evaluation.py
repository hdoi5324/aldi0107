...
class Detectron2COCOIOUEvaluatorAdapter(Detectron2COCOEvaluatorAdapter):
    """A Detectron2COCOEvaluatorAdapter for AP25."""
...
class COCOeval_opt_IOU(COCOeval_opt):
    """A COCOeval_opt to calculate AP25."""
...
class Detectron2COCORecallEvaluatorAdapter(Detectron2COCOEvaluatorAdapter):
    
    """A Detectron2COCOEvaluatorAdapter that includes recall AR, AR50@1, AR50@100, AR50@1000."""
        ...
        for task in sorted(tasks):
            ...
            coco_eval = (
                _evaluate_predictions_on_coco(
                    ...
                    task,
                    kpt_oks_sigmas=self._kpt_oks_sigmas,
                    cocoeval_fn=COCOeval_opt_Recall,
                    img_ids=img_ids,
                    max_dets_per_image=self._max_dets_per_image,
                )
                if len(coco_results) > 0
...
class COCOeval_opt_Recall(COCOeval_opt):
    """A COCOeval_opt to calculate AP25."""
    def __init__(self, cocoGt=None, cocoDt=None, iouType='segm'):
        super().__init__(cocoGt=cocoGt, cocoDt=cocoDt, iouType=iouType)
    ...
    def summarize(self):
        ...
        def _summarizeDets():
            ...
            stats[9] = _summarize(0, iouThr=0.5, maxDets=self.params.maxDets[0])
            stats[10] = _summarize(0, iouThr=0.5, maxDets=self.params.maxDets[1])
            stats[11] = _summarize(0, iouThr=0.5, maxDets=self.params.maxDets[2])
