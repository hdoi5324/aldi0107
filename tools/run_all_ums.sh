...
do
  ${script_start}run_model_selection.py --config-file "${hd}/GitHub/${gd_source}/outputs/${model_dataset}/config.yaml" OUTPUT_DIR ${hd}/GitHub/${gd_source}/outputs/${model_dataset}/ MODEL_SELECTION.N_SAMPLE ${sample_size} MODEL_SELECTION.N_TRANSFORMED_SOURCE ${transformed_source} MODEL_SELECTION.N_PERTURBATIONS 3 MODEL_SELECTION.PERTURB_TYPE "DAS"
  #${script_start}modelSele_DAS.py --config-file "${hd}/GitHub/${gd_source}/outputs/${model_dataset}/config.yaml" OUTPUT_DIR "${hd}/GitHub/${gd_source}/outputs/${model_dataset}/" MODEL_SELECTION.N_SAMPLE ${sample_size} MODEL_SELECTION.N_TRANSFORMED_SOURCE ${transformed_source} MODEL_SELECTION.N_PERTURBATIONS 3 MODEL_SELECTION.PERTURB_TYPE "DAS"
...
datasets=("cityscapes/cityscapes_aldi_last" "imosauv/imosauv_ALDI_last" "urchininf/urchininf_ALDI_last" "sim10k/sim10k_ALDI_last" "sim10k/sim10k_baseline_strongaug_ema")
